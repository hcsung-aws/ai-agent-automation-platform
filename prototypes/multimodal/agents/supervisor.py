...
def _get_cloudwatch_agent():
    """CloudWatch Agent를 지연 초기화합니다."""
    global _cloudwatch_agent
    if _cloudwatch_agent is None:
        from cloudwatch_agent import create_cloudwatch_agent
        _cloudwatch_agent = create_cloudwatch_agent()
    return _cloudwatch_agent
...
@tool
def ask_cloudwatch_agent(query: str) -> str:
    """CloudWatch 모니터링 Agent에게 질문합니다.
    
    CloudWatch 알람 조회, 메트릭 시각화, 알람 상태 분석을 담당합니다.
    
    Args:
        query: CloudWatch 관련 질문
    
    Returns:
        CloudWatch Agent의 응답
    """
    agent = _get_cloudwatch_agent()
    return str(agent(query))
...
### CloudWatch Agent (ask_cloudwatch_agent)
담당 영역:
- CloudWatch 알람 목록 조회
- 메트릭 시각화 이미지 생성
- 알람 상태 요약 및 분석
...
사용 예시:
- "현재 ALARM 상태 알람 보여줘" → ask_cloudwatch_agent
- "EC2 CPU 사용률 그래프 그려줘" → ask_cloudwatch_agent
- "알람 전체 현황 요약해줘" → ask_cloudwatch_agent
...
2. 복합 요청 (수집 + 분석/요약/비교):
   - 먼저 데이터 수집 Agent를 호출하여 원본 데이터 확보
   - 그 결과를 분석 Agent에게 전달하여 심층 분석 수행
   - 두 Agent의 결과를 종합하여 최종 응답 생성
3. 판단 기준: 사용자가 "분석", "비교", "트렌드", "요약 정리", "심층" 등을 요청하면 분석 Agent도 반드시 호출
4. 불명확: 사용자에게 명확화 요청
...
- 하위 Agent가 제공한 URL/링크/이미지를 그대로 보존 (요약 시에도 생략 금지)
"""
