...
| 피드백 수집 | ❌ 미구현 | 좋아요/싫어요 없음 |
| 자동 개선 | ❌ 미구현 | 수동 KB 업데이트만 가능 |
...
|------|------|
| 피드백 UI | 응답별 👍/👎 버튼 + 코멘트 |
| 피드백 저장 | DynamoDB feedback 테이블 |
| 피드백 조회 API | GET /api/feedback |
...
| 작업 | 설명 |
|------|------|
| 실패 패턴 분석 | 자주 실패하는 질문 유형 클러스터링 |
| KB 문서 초안 생성 | 실패 케이스 → 가이드 문서 자동 생성 |
| System Prompt 제안 | 개선된 지침 제안 |

---

### v2.0 - Agent Builder Agent (3주) ⭐ 핵심
...
| "실패 로그 분석해서 개선해줘" | 로그 분석 → KB/Prompt 수정 |
...
**즉시 시작 가능**: v2.0 Agent Builder (Kiro CLI 확장)
