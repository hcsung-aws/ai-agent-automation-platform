...
Chainlit은 메시지별 피드백 기능을 기본 제공합니다.

### `templates/local/app.py` 수정

```python
"""AIOps 스타터 킷 - Chainlit UI (피드백 포함)."""
import json
...
Chainlit 설정 파일 `.chainlit/config.toml`에서 피드백을 활성화합니다:
...
```json
{"timestamp": "2026-02-09T10:30:00", "message_id": "abc123", "rating": "negative", "comment": "직원 ID 형식을 몰랐어요"}
```
